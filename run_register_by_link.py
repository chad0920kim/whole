...
EXCLUDED_CATEGORY_KEYWORDS = [
    ...
    '담배', '전자담배',  # 담배 판매 권한 필요
    # KC인증 필요 카테고리 추가 (카테고리 변경 시 발견된 문제)
    ...
    for keyword in unique_keywords:
        ...
        for cat in _leaf_category_cache:
            ...
            if any(ex in whole_name.lower() for ex in EXCLUDED_KEYWORDS):
                continue
            # KC 인증/결제수단 제한 카테고리 ID 필터링
            if cat_id in KC_REQUIRED_CATEGORY_IDS or cat_id in RESTRICTED_PAYMENT_CATEGORY_IDS:
                continue
        ...
        # 제외 카테고리 필터링 (키워드 + ID 기반)
        partial_matches = [
            cat for cat in partial_matches
            if not any(ex in cat.get('wholeCategoryName', '').lower() for ex in EXCLUDED_KEYWORDS)
            and str(cat.get('id', '')) not in KC_REQUIRED_CATEGORY_IDS
            and str(cat.get('id', '')) not in RESTRICTED_PAYMENT_CATEGORY_IDS
        ]
