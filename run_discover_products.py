...
def main():
    ...
    # 상품 선택
    while True:
        choice = input("등록할 상품 번호 (q=종료, r=재검색): ").strip().lower()
        ...
        if choice == 'q':
            ...
            if 0 <= idx < len(products):
                ...
                # 확인
                print(f"\n선택: [{selected['item_no']}] {selected['name'][:45]}")
                print(f"  도매가: {selected['price']:,}원 → 판매가: {selected['margin_price']:,}원")
                confirm = input("등록하시겠습니까? (y/Enter=등록, n=취소): ").strip().lower()
